...
static void ini_anim_v3(t_data2 *data)
{
    ...
    free_s(path_p);
    if (!j)
        (put(RED"cant open player sprite\n"RESET), exit_all_v2(data));
    data->starter = rand() % 4;
    i = -1;
    while (++i < 8)
    {
        ...
        free(path_p);
        if (!j)
            (put(RED"cant open enemy n*%d\n"RESET, data->starter), exit_all_v2(data));
    }
    path_p = str("img/txt_grass_%d.xpm", data->starter);
    j = helper_texture(data, path_p, &data->i_ground[1], 1);
    free_s(path_p);
    if (!j)
        (put(RED"cant open grass sprite\n"RESET), exit_all_v2(data));
}
...
void ini_sprites(t_data2 *data)
{
    ...
    helper_texture(data, "img/txt_exit2.xpm", &data->i_exit[1], 0);

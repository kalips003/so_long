...
void	ft_background(t_data2 *data, int sw)
{
	...
	ft_put_rectangle(data, (int [2]){data->buffer.sz_x - BALL_START_X - OFFSET_BALL * data->num_holding, data->buffer.sz_y - BLACK_BOT}, \
		(int [2]){data->buffer.sz_x, data->buffer.sz_y}, ft_black);
	ft_put_rectangle(data, (int [2]){END_X_BB - 2, data->buffer.sz_y + BOT_STAMINA}, \
		(int [2]){data->buffer.sz_x - BALL_START_X - OFFSET_BALL * data->num_holding + 1, data->buffer.sz_y}, ft_black);
	ft_stamina(data);
...
void	draw_frame_scaled(t_data2 *data, t_img img, int xyfe[3], unsigned int (*color)(void))
{
	...
	while (++f.i < img.sz_x)
	{
		...
		while (++f.j < img.sz_y)
		{
			...
			if (color)
				f.pixel_color = color();
			ft_put_square(data, (int [2]){xyfe[0] + f.i * xyfe[2], xyfe[1] + f.j * xyfe[2]}, xyfe[2], f.pixel_color);
		}
	}
}
...
void	ft_draw_score(t_data2 *data)
{
	...
	while (score > 9)
	{
		draw_frame_scaled(data, data->i_numbers[score % 10], (int [3]){i, start_y_posi, 4}, color);
		i += DECIMAL_SCORE;
		score /= 10;
	}
	draw_frame_scaled(data, data->i_numbers[score % 10], (int [3]){i, start_y_posi, 4}, color);

...
void put_background_to_img(t_data2 *data)
{
    int x;
    int y;
...
void put_foreground_to_img(t_data2 *data)
{
    int i;
...
/////////////////////////////////////////////////////////////////////////////////////
static void ini_anim2(t_data2 *data)
{
    char *path_p;

    path_p = str("img/player_b/player_%d.xpm", 0);
    data->img_player = mlx_xpm_file_to_image(data->mlx, path_p, &data->wid, &data->hei);
    free(path_p);
    if (!data->img_player)
    {
    ...
    {
        // random starter
        path_p = str("img/enemy/%d/enemy_%d.xpm", 0, i);
        data->img_pika[i] = mlx_xpm_file_to_image(data->mlx, path_p, &data->wid, &data->hei);
        free(path_p);
        if (!data->img_pika[i])
        {
...
void ini_texture2(t_data2 *data)
{
	int i;
...
// void f_put_img_to_buffer(t_data2 *data, t_coor xy, )
// {
